...
| `@arrow` | y x v sound | Optional flying arrow (one per room). **Mutually exclusive with `@rope`.** `y` = entity pixel Y (tile row = `y >> 3`, same as Spectrum `spec // 2`); `x` = start column (0–255); `v` = `1` (LTR) or `-1` (RTL); `sound` = column where launch SFX fires. Baked **`arrow_init`** + **`arrow_update_a`** @ `$1B48` (sprite frame 8), **`arrow_update_b`** @ `$1DA8` (above chr 52 UDG); restores trail cells from map snapshot each tick. Meta flag at tail +99. |
